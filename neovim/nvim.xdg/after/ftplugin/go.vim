...
" Snippets
iabbrev xtable tests := []struct{<cr>msg string<cr>wantErr string<cr>}{<cr>{<cr>msg: "ok",<cr>wantErr: "",<cr>},<cr>}<cr><cr>for i := range tests {<cr>tt := tests[i]<cr>t.Run(tt.msg, func(t *testing.T) {<cr>TODO<cr>})<cr>}
