...
function name2tags(name, scr)
  ...
  for s = scr, nscr do
    for id, tg in ipairs(capi.screen[s]:tags()) do
...
-- send function can be replaced with awful.tag.move, so it is not needed
-- anymore
local send = awful.tag.move
...
function set(t, args)
  ...
  local scr = args.screen or (not t.screen and preset.screen) or t.screen or capi.mouse.screen
  ...
  local props = {
    ...
    icon = select{ args.icon and capi.image(args.icon), preset.icon and capi.image(preset.icon), awful.tag.getproperty(t,"icon"), config.defaults.icon and capi.image(config.defaults.icon) },
  ...
  local sel_idx = (sel and tag2index(sel,scr)) or 0 --TODO: what happens with rel_idx if no tags selected
  local t_idx = tag2index(t,scr)
...
-- delete function can be provided by awful.tag.delete
function del (args)
  awful.tag.delete(args)
end
    ...
    for i, t in ipairs(capi.screen[s]:tags()) do
      ...
      if #clients == sticky then
        if not awful.tag.getproperty(t,"persist") and awful.tag.getproperty(t,"used") then
          ...
          elseif not t.selected and awful.tag.getproperty(t,"visited") then 
...
function getpos(pos)
  ...
  for i = 1, capi.screen.count() do
    local s = awful.util.cycle(capi.screen.count(), scr + i - 1)
    for j, t in ipairs(capi.screen[s]:tags()) do
...
function completion(cmd, cur_pos, ncomp, sources, matchers)
  ...
  local get_source = {
    ...
    existing = function()
      local ret = {}
      for i = 1, capi.screen.count() do
        local s = awful.util.cycle(capi.screen.count(), capi.mouse.screen + i - 1)
...
for s = 1, capi.screen.count() do
  awful.tag.attached_connect_signal(s, "property::visited", sweep)
  awful.tag.attached_connect_signal(s, "property::deserted", sweep)
